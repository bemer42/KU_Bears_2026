function Total_Cells = stem_cell_movement(alpha_z, zp, bcType)
...
load('Average_Values', 'B_avg', 'A_avg', 'z_old')
S_avg = B_avg.*sqrt(15)./A_avg./sqrt(B_avg + 30);
s_star = interp1(z_old, S_avg, z, 'pchip', 'extrap');
s_star = L_s * (s_star - s_0) / (L_s - s_0 + eps);    
v0 = 1e-3; w = .5;
VS = -v0 * tanh((S - repmat(s_star.', size(S,1), 1)) / w);
...
Total_Cells = trapz(z, trapz(s, P, 1));
